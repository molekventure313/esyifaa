...
function scrollToTest(e) {
  e?.preventDefault();
  const el = document.getElementById('test-air-tawar');
  if (el) el.scrollIntoView({ behavior: 'smooth' });
}

// ─── WhatsApp Button ──────────────────────────────────────────────────────────
function WAButton({ label = '🟢 Hubungi ESyifaa Sekarang', id = 'cta-wa', size = 'large', href = '#' }) {
  const handleClick = () => { try { window.fbq('track', 'Lead'); } catch (_) {} };
  const isLarge = size === 'large';
  return (
    <a id={id} href={href} target="_blank" rel="noopener noreferrer" onClick={handleClick}
      style={{
      ...
      onMouseEnter={e => { e.currentTarget.style.transform = 'translateY(-2px)'; e.currentTarget.style.boxShadow = '0 12px 30px rgba(37,211,102,0.6)'; }}
      onMouseLeave={e => { e.currentTarget.style.transform = 'translateY(0)'; e.currentTarget.style.boxShadow = '0 8px 25px rgba(37,211,102,0.5)'; }}
    >
...
function FAQItem({ q, a, isOpen, onToggle }) {
  return (
    <div style={{ background: '#042E23', border: isOpen ? '2px solid #FDE047' : '1px solid rgba(253,224,71,0.2)', borderRadius: '12px', overflow: 'hidden' }}>
      <button onClick={onToggle} style={{ width: '100%', padding: '1rem 1.25rem', background: 'transparent', border: 'none', textAlign: 'left', fontWeight: 800, fontSize: '0.95rem', color: isOpen ? '#FDE047' : '#FEF3C7', cursor: 'pointer', display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: '1rem', fontFamily: 'var(--font-inter), -apple-system, sans-serif' }}>
        <span>{q}</span>
        <span style={{ fontSize: '1.4rem', color: '#FDE047', flexShrink: 0, lineHeight: 1 }}>{isOpen ? '−' : '+'}</span>
      </button>
      {isOpen && (
        <div style={{ padding: '0 1.25rem 1rem', fontSize: '0.875rem', color: '#D1FAE5', lineHeight: 1.65, borderTop: '1px solid rgba(253,224,71,0.15)', paddingTop: '0.85rem' }}>
          {a}
        </div>
      )}
    </div>
...
export default function WaPage() {
  ...
  return (
    <main style={{ minHeight: '100vh', background: '#042E23', fontFamily: ff }}>
      ...
      <section style={{ background: '#031E17', padding: '2.5rem 1rem', textAlign: 'center' }}>
        <div style={{ maxWidth: '600px', margin: '0 auto' }}>
          <div style={{
            ...
            <div style={{ fontSize: '1.75rem', marginBottom: '0.65rem' }}>⚠️</div>
            <h3 style={{ fontWeight: 800, fontSize: '1.15rem', color: '#FEF3C7', marginBottom: '0.6rem', lineHeight: 1.3 }}>
              Ada Tindakbalas Selepas Minum Air Tadi?
            </h3>
      ...
      <section style={{ background: '#042E23', padding: '3.5rem 1rem' }}>
        <div style={{ maxWidth: '680px', margin: '0 auto' }}>
          <div style={{ textAlign: 'center', marginBottom: '2rem' }}>
            ...
            <h2 style={{ fontSize: 'clamp(1.35rem, 3.5vw, 2rem)', fontWeight: 800, color: '#FEF3C7', letterSpacing: '-0.02em', lineHeight: 1.25, marginTop: '0.4rem', marginBottom: '0.6rem' }}>
              8 Tanda Anda Mungkin Mengalami Gangguan Spiritual
            </h2>
            <p style={{ fontSize: '0.9rem', color: '#A7F3D0', lineHeight: 1.65, margin: 0 }}>
              Jika anda alami 3 atau lebih simptom ini, cuba test air tawar dahulu untuk kepastian.
            </p>
          </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.65rem' }}>
            {SYMPTOMS.map((s, i) => (
              <div key={i} style={{
                ...
                padding: '0.85rem 1.1rem', border: '1px solid rgba(74,222,128,0.15)',
              }}>
                <span style={{ width: '22px', height: '22px', borderRadius: '50%', background: 'rgba(253,224,71,0.15)', border: '2px solid #FDE047', display: 'inline-flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, fontSize: '0.75rem', color: '#FDE047', fontWeight: 900 }}>✓</span>
          ...
          <div style={{ textAlign: 'center', marginTop: '2rem' }}>
            <p style={{ fontSize: '0.9rem', color: '#FCA5A5', fontWeight: 700, marginBottom: '1.25rem' }}>
              Semakin lama dibiarkan, semakin sukar untuk ditangani.
            </p>
            <button
              onClick={scrollToTest}
              style={{
                ...
                fontSize: '1rem', fontWeight: 800, color: '#042E23',
                background: 'linear-gradient(180deg, #FDE047 0%, #EAB308 100%)',
                borderRadius: '50px', border: '2px solid #FEF08A',
        ...
        <div style={{ maxWidth: '860px', margin: '0 auto', textAlign: 'center' }}>
          ...
          <div style={{ maxWidth: '520px', margin: '0 auto' }}>
            <div style={{
              ...
              boxShadow: '0 20px 60px rgba(0,0,0,0.4)', marginBottom: '1.5rem',
      ...
      <section style={{ background: '#FFFFFF', color: '#0F172A', padding: '3.5rem 1rem' }}>
        <div style={{ maxWidth: '960px', margin: '0 auto', textAlign: 'center' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '1.25rem', alignItems: 'start' }}>
            {['/images/testimonials/testimoni_1.jpg', '/images/testimonials/testimoni_2.jpg', '/images/testimonials/testimoni_3.jpg', '/images/testimonials/testimoni_part2_1.jpg'].map((src, i) => (
      ...
      {/* ══════════════════════════════════════════
          SECTION 7 — KENAPA ESYIFAA
      ══════════════════════════════════════════ */}
      <section style={{ background: '#031E17', padding: '3.5rem 1rem' }}>
        <div style={{ maxWidth: '680px', margin: '0 auto', textAlign: 'center' }}>
          <span style={{ fontSize: '0.75rem', fontWeight: 800, color: '#FDE047', textTransform: 'uppercase', letterSpacing: '0.1em' }}>Kenapa ESyifaa?</span>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem', textAlign: 'left', marginBottom: '2rem' }}>
            {[
              ...
              <div key={i} style={{ display: 'flex', gap: '1rem', alignItems: 'flex-start', background: '#042E23', border: '1px solid rgba(253,224,71,0.15)', borderRadius: '12px', padding: '1.25rem' }}>
                <span style={{ fontSize: '1.75rem', flexShrink: 0, background: 'rgba(253,224,71,0.1)', borderRadius: '10px', padding: '0.35rem', display: 'inline-flex', lineHeight: 1 }}>{p.icon}</span>
                <div>
      ...
      <section style={{ background: '#0B382D', padding: '3.5rem 1rem' }}>
        <div style={{ maxWidth: '960px', margin: '0 auto', textAlign: 'center' }}>
          ...
          <p style={{ fontSize: '0.95rem', color: '#FFFFFF', marginBottom: '2rem', opacity: 0.9, lineHeight: 1.6 }}>Jom baca apa kata mereka yang dah cuba rawatan ESyifaa 👇</p>
          ...
          <p style={{ marginTop: '1.5rem', fontSize: '0.85rem', color: '#A7F3D0', fontStyle: 'italic', lineHeight: 1.6 }}>Semua testimoni adalah daripada pesakit sebenar ESyifaa. Alhamdulillah.</p>
        </div>
      </section>
      ...
      <section style={{ background: '#042E23', padding: '3.5rem 1rem' }}>
        <div style={{ maxWidth: '640px', margin: '0 auto' }}>
          <div style={{ textAlign: 'center', marginBottom: '2rem' }}>
            <span style={{ fontSize: '0.75rem', fontWeight: 800, color: '#FDE047', textTransform: 'uppercase', letterSpacing: '0.1em' }}>❓ Soalan Lazim</span>
