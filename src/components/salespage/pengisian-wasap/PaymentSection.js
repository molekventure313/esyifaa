...
// ─── WA Rotator (read current slot, no FPX) ───────────────────────────────────
...
export default function PengisianWasapPaymentSection() {
  const [waLink, setWaLink] = useState(buildWaLink(FALLBACK_NUMBER));

  useEffect(() => {
    // Read current rotated number from localStorage (same pool as WACtaSection)
    const initWaLink = async () => {
      try {
        ...
        const numbers = (json.success && json.data?.length > 0)
          ...
          : [FALLBACK_NUMBER];
        // Read current index without advancing (WACtaSection handles the advance)
      ...
      <div style={{ maxWidth: '860px', margin: '0 auto' }}>
        ...
        <div style={{
          ...
          <div style={{ background: 'rgba(255,255,255,0.1)', borderRadius: '999px', height: '10px', overflow: 'hidden', marginBottom: '0.65rem' }}>
            <div style={{ width: `${slotPct}%`, height: '100%', background: 'linear-gradient(90deg, #EF4444, #DC2626)', borderRadius: '999px' }} />
          </div>
          ...
          <div style={{
            ...
            <div style={{ padding: '2rem', textAlign: 'center', borderBottom: '1px solid rgba(74,222,128,0.2)' }}>
              ...
              <div style={{ marginBottom: '0.4rem' }}>
                <span style={{ fontSize: '0.9rem', color: '#F87171', marginRight: '0.5rem' }}>Harga selepas slot habis:</span>
                <span style={{ fontSize: '1.15rem', color: '#F87171', textDecoration: 'line-through', fontWeight: 700 }}>RM120</span>
              </div>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.75rem', marginBottom: '0.35rem' }}>
                <div style={{ background: 'rgba(239,68,68,0.15)', border: '1px solid rgba(239,68,68,0.4)', color: '#FCA5A5', padding: '0.2rem 0.65rem', borderRadius: '50px', fontSize: '0.72rem', fontWeight: 800, textTransform: 'uppercase' }}>
                  Harga Sekarang
                </div>
            ...
            <div style={{ padding: '1.75rem 2rem' }}>
              {INCLUDES.map((item, i) => (
                <div key={i} style={{ display: 'flex', alignItems: 'flex-start', gap: '0.75rem', marginBottom: i < INCLUDES.length - 1 ? '0.85rem' : 0 }}>
                  <span style={{ fontSize: '1.15rem', flexShrink: 0 }}>{item.icon}</span>
                  <span style={{ fontSize: '0.9rem', lineHeight: 1.5, color: item.highlight ? '#FDE047' : '#D1FAE5', fontWeight: item.highlight ? 700 : 400 }}>
                    {item.text}
                  </span>
                </div>
              ))}
